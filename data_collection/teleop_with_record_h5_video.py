...
This script combines Quest VR controller teleoperation with simultaneous robot trajectory recording, saving camera data as H.264-encoded videos.
"""
...
class TrajectoryRecorder:
    def __init__(self, output_file, camera_config=None, save_path="./data"):
        self.timestamps = []
        self.q_list = []
        self.theta_list = []
        self.dq_list = []
        ...
        self.camera_images_list = {}  # Store frames temporarily
        self.action_list = []

        self.output_file = output_file
        self.save_path = save_path  # Path to save videos
    ...
    def add_state(self, robot_states, gripper_states, quest_input=None):
        ...
        try:
            ...
            for i, (image, _, _) in enumerate(camera_data):
                ...
                # Store image as RGB (ensure correct format for OpenCV)
                image_rgb = cv2.cvtColor(np.array(image, copy=True), cv2.COLOR_BGR2RGB)
                self.camera_images_list[cam_key].append(image_rgb)
    ...
    def save_trajectory(self, task):
        """Save trajectory data to HDF5 file and camera data as H.264-encoded videos"""
        self.align_frames()
        self.logger.info(f"Saving trajectory to {self.output_file} and videos to {self.save_path}...")
        ...
        with h5py.File(self.output_file, 'w', libver='latest', rdcc_nbytes=1024*1024*100) as hf:
            ...
            # Store video file paths in HDF5 attributes
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            for i in range(self.num_cameras):
                cam_name = f'cam{i+1}'
                video_filename = os.path.join(self.save_path, f"{cam_name}_{timestamp}.mp4")
                hf.attrs[f'{cam_name}_video_path'] = video_filename

        # Save videos asynchronously with H.264 encoding
        def save_video(images, cam_name, video_filename, fps=30):
            try:
                height, width, _ = images[0].shape
                fourcc = cv2.VideoWriter_fourcc(*'H264')  # Use H.264 codec
                out = cv2.VideoWriter(video_filename, fourcc, fps, (width, height))
                if not out.isOpened():
                    self.logger.error(f"Failed to open VideoWriter for {cam_name}. Check FFmpeg support.")
                    return
                for image in images:
                    out.write(image)
                out.release()
                self.logger.info(f"Saved H.264 video for {cam_name} to {video_filename}")
            except Exception as e:
        ...
        for i in range(self.num_cameras):
            ...
            thread = threading.Thread(target=save_video, args=(images, cam_name, video_filename, 30))
            threads.append(thread)
            thread.start()
        
        # Wait for all video saving to complete
        for thread in threads:
            thread.join()
        
        self.logger.info(f"Task: {task}, Frames: {len(self.timestamps)}, Saved to: {self.output_file}, Videos saved to: {self.save_path}")

# The rest of the script (get_cur_pose, main, and argument parsing) remains unchanged
...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30, save_path="./data"):
    ...
    recorder = TrajectoryRecorder(output_file, camera_config, save_path)
    ...
    try:
        ...
        logger.info(f"Trajectory saved to {recorder.output_file}, Videos saved to {recorder.save_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default="./data", help="Path to save video data")
